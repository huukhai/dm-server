...
public class Node implements XmlIO{
   ...
   /** Returns the element with the given namespace and name. If the
	...
	do {
        ...
        // GROSS HACK!!!! - get rid of this tight coupling with the wap stuff
        if((event.getType()==Xml.WAP_EXTENSION)&&
           ((WapExtensionEvent)event).getContent() instanceof Document)
        {
            addChild(Xml.ELEMENT,
                     ((Document)((WapExtensionEvent)event).getContent()).getRootElement());
        }
        else
        {
